...
def get_mut_white_list(mut2samples, muts_black_list_file):
    ...
    all_muts = sorted(mut2samples.keys())
        ...
        if mut in mut_white_list and mut in mut2neighbors:
            ...
            for neigh_mut in neighbors:
                ...
                if neigh_mut > mut and neigh_mut in mut_white_list:
                    edge_weight = compute_weight(mut2samples, sample2class, list([mut]), list([neigh_mut]), tot_class0,
                                                 tot_class1)
...
def best_solution_from_edge(mut2samples, sample2class, tot_class0, tot_class1, mut2neighbors, mut_white_list,
                            starting_edge):
    old_sol = starting_edge
    new_sol, new_val = extend_solution(mut2samples, sample2class, tot_class0, tot_class1, mut2neighbors, mut_white_list,
                                       old_sol)
    ...
    while len(new_sol) > len(old_sol) and len(new_sol) < max_size:
        # print "HERE!"
        old_sol = new_sol
        new_sol, new_val = extend_solution(mut2samples, sample2class, tot_class0, tot_class1, mut2neighbors,
                                           mut_white_list, old_sol)
    # print "new_sol: "+str(new_sol)+", new val: "+str(new_val)
    # print "old_sol: "+str(old_sol)
...
network_file = 'hint+hi2012_edges.txt'
mut_matrix_file = '1VS10/damokle_input_80P_class_and_snvs/filtered_snvs_80P.tsv'
directory = "1VS10/damokle_input"

#class_file = 'filesForRunningDamokle/damokle_input_80P/OVis0_COADREADis1_file.txt'
...
for filename in os.listdir(directory):
    ...
    if os.path.isfile(file_path):
        ...
        last_part = class_file.split('/')[-1]
        print(last_part)
        print(x)
        output_file = f'/Users/hamidkhan/PycharmProjects/lastModification/damokle_output_1vs10_k13_minfreq001_threshold01_permu100/{last_part}'
        ...
        edges = get_edges_above_threshold(mut2samples, sample2class, total_samples_class0, total_samples_class1, mut2neighbors,
                                          mut_white_list)
        print(len(edges))
        # print edges
        ...
        for starting_edge in edges:
            best_sol_edge, best_val_edge = best_solution_from_edge(mut2samples, sample2class, total_samples_class0,
                                                                   total_samples_class1, mut2neighbors, mut_white_list,
                                                                   starting_edge)
        ...
        for i in range(num_permutations):
            permuted_dataset_mut2samples = generate_permuted_dataset(mut2samples, sample2class, samples_with_muts_AND_class,
                                                                     mut_white_list)
            # get all the pairs of edges with discrepancy above a given value
            edges = get_edges_above_threshold(permuted_dataset_mut2samples, sample2class, total_samples_class0,
                                              total_samples_class1, mut2neighbors, mut_white_list)
            # now keep adding nodes to a starting edge until you cannot improve
            best_val_perm = 0.0

            for starting_edge in edges:
                best_sol_edge_perm, best_val_edge_perm = best_solution_from_edge(permuted_dataset_mut2samples, sample2class,
                                                                                 total_samples_class0, total_samples_class1,
                                                                                 mut2neighbors, mut_white_list, starting_edge)
        ...
        for i in range(num_permutations):
            ...
            # get all the pairs of edges with discrepancy above a given value
            edges = get_edges_above_threshold(mut2samples, permuted_dataset_samples2class, total_samples_class0,
                                              total_samples_class1, mut2neighbors, mut_white_list)
            # now keep adding nodes to a starting edge until you cannot improve
            best_val_perm = 0.0

            for starting_edge in edges:
                best_sol_edge_perm, best_val_edge_perm = best_solution_from_edge(mut2samples, permuted_dataset_samples2class,
                                                                                 total_samples_class0, total_samples_class1,
                                                                                 mut2neighbors, mut_white_list, starting_edge)
                if best_val_edge_perm > best_val_perm:
                    best_val_perm = best_val_edge_perm
